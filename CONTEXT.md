...
19. **Audio crackle/dropouts fix:** the audio stutter the user heard is mpv's audio thread getting starved when sixel encoding (UI thread) + per-frame `screenshot-to-file` (mpv thread) spike CPU on WSL — *not* bitrate (`bestaudio` ≈130k opus is already max). Engine now adds `--audio-buffer` (`_audio_buffer()`, `BOXTUBE_AUDIO_BUFFER` default 0.6s, also a Settings choice), `--screenshot-high-bit-depth=no`, `--cache=yes`, `--demuxer-readahead-secs=20`; the capture loop polls volume every 12th tick instead of every frame to cut IPC contention. Raise the buffer / lower FPS if it persists. **A/V sync:** video is sampled (screenshot→encode→transmit) so it shows a beat after the real-time audio; `--audio-delay` (`_audio_delay()`, `BOXTUBE_AUDIO_DELAY` default 0, Settings choice) holds audio back to match. **FPS note:** `BOXTUBE_PLAYER_FPS` caps at 60 but the sixel pipeline can't sustain it and YouTube is ~24–30fps, so >24 just duplicates frames + risks crackle — not real 60fps.
20. **Resume / watch position:** `progress.py` (per-video JSON store next to the config; `BOXTUBE_PROGRESS` override; `record`/`resume_at`/`clear`/`load`, `_LOCK`-guarded atomic write, prune to `MAX_ENTRIES`=500). Player: `_on_started` → `_maybe_resume()` (absolute seek + `Resumed at M:SS` toast); capture loop persists every ~5s via `_save_progress()`; also saves on `action_close` and clip switch (`_switch_to`); EOF → `_mark_finished()` → `progress.clear`. Thresholds: `MIN_RESUME_SECONDS`=15 (skip the first 15s), finished = within 20s of end or ≥97% (forgotten so it restarts). Tests: `test_progress.py` + a resume test in `test_player_screen.py` (fixtures isolate `BOXTUBE_PROGRESS`). Three-feature batch requested: resume (done), search history+suggestions (done), comments pane (next); aesthetic features held for last.
21. **Search history + suggestions:** `history.py` (recent queries JSON next to config; `BOXTUBE_HISTORY` override; `add`/`recent`/`clear`, `_LOCK`, atomic write, case-insensitive move-to-front dedupe, cap `MAX_ITEMS`=50). `app.HistorySuggester(Suggester)` (`use_cache=False`, `case_sensitive=False`) completes the search `Input` inline from history (empty box → latest query; prefix → most recent match; `→`/`End` accepts the ghost text). `on_input_submitted` calls `history.add(query)` (typed searches only — chips don't record). Tests: `test_history.py` (store + suggester) + a submit-records-history test in `test_app.py`.
22. **Comments pane:** `youtube.fetch_comments(video_id, limit=40, cookies=None, sort='top')` runs yt-dlp `--dump-single-json --write-comments --ignore-no-formats-error --extractor-args youtube:comment_sort=…;max_comments=N,all,0,0` (+`--js-runtimes` if found), parses `info['comments']` → `Comment` dataclass (`author`/`text`/`likes`/`is_uploader`/`is_favorited`/`time_text`; `likes_str`; text `\r\n`→`\n`). Flat top-level list (0 replies), public (no cookies sent). `comments_screen.py`: `CommentsScreen(ModalScreen)` opens instantly with "Loading…", a `@work(thread=True)` worker calls fetch then `self.app.call_from_thread` (NOT `self.` — Screen has no `call_from_thread`) → mounts `CommentRow`s / empty / error. App: `c` binding → `action_comments` (needs a highlighted `Video`). TCSS `#comments-dialog`/`.comment-row` etc. Tests: `test_youtube.py` (fetch parsing/command/empty/error) + `test_comments_screen.py`. **All three requested features now done; aesthetic features (channel avatar, accent-color settings) remain held for last.**
...
`BOXTUBE_AUDIO_BUFFER` (0.6), `BOXTUBE_AUDIO_DELAY` (0.0),
`BOXTUBE_PROGRESS` (resume store path), `BOXTUBE_HISTORY` (search history path).
